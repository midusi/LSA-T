...
def main():
    ...
    sub_files = [f for f in input_path.iterdir() if str(f).endswith('.vtt') if str(f) not in excluded]
    ...
    for vid_idx, filename in enumerate(sub_files):
        ...
        with VideoFileClip(str(Path(input_path / f"{name}.mp4").resolve())) as video:
            for i, (start, end, sub) in enumerate(subs):
                ...
                if not (outdir / f"{i}.json").exists():
                    ...
                    newvid.write_videofile(str(outdir / f"{i}.json"), audio=False)
